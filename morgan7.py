...
def extract_speaker_embeddings(audio_paths, processor, model, device="cpu"):
    model.to(device)
    ...
    with torch.no_grad():  # Disable gradient calculation
        for audio_path in audio_paths:
            ...
            inputs = processor(audio=waveform.squeeze(), return_tensors="pt", sampling_rate=16000)  # Adjust sampling_rate if needed
            input_ids = inputs.input_ids.to(device)
            attention_mask = inputs.attention_mask.to(device)

            # Extract embeddings (this is the key part)
            # The exact layer to extract from might need experimentation
            encoder_outputs = model.speecht5.encoder(input_ids=input_ids, attention_mask=attention_mask)
            embedding = encoder_outputs.last_hidden_state[:, 0, :]  # Take the embedding from the first token
    ...
    if embeddings:
        ...
        return None  # Handle the case of no embeddings
...
if __name__ == "__main__":
    ...
    speaker_embeddings_dataset = load_dataset(
        "Matthijs/cmu-arctic-xvectors", split="validation"
    )
    speaker_embeddings = torch.tensor(
        speaker_embeddings_dataset[0]["xvector"]
    ).unsqueeze(0)
    ...
    # Debugging prints
    print(f"Full 'inputs' object: {inputs}")  # Print the entire object
    if hasattr(inputs, "input_ids"):
        input_ids = inputs.input_ids
    else:
        print("'input_ids' not found as attribute.  Checking as key")
        if "input_ids" in inputs:
            input_ids = inputs["input_ids"]
        else:
            print("'input_ids' not found in inputs")
            input_ids = None  # set to None to avoid errors
    if hasattr(inputs, "attention_mask"):
        ...
        if "attention_mask" in inputs:
            ...
            print("'attention_mask' not found in inputs")
            attention_mask = None
